...
export const viewInvoice = (order) => {
  ...
  pdf.setFont("times", "bold");
  pdf.text(25, 50, "Billed to");
  pdf.setFont("times", "normal");
  pdf.text(25, 55, `${order.customer.name}`);
  pdf.text(25, 60, `${addressString}`);
  pdf.text(25, 65, `${order.customer.email}`);
  pdf.text(25, 70, `${order.customer.phone}`);
  ...
  pdf.line(
    0,
    startY + order.products.length * 10 + 10,
    210,
    startY + order.products.length * 10 + 10
  );

  // Sub Total amount
  pdf.text(120, startY + order.products.length * 10 + 20, "Sub Total:");
  pdf.text(
    160,
  ...
  // Discount amount
  pdf.text(120, startY + order.products.length * 10 + 25, "Discount:");
  pdf.text(160, startY + order.products.length * 10 + 25, `$${Discount}.00`);

  // Tax amount
  pdf.text(120, startY + order.products.length * 10 + 30, "Tax:");
  pdf.text(160, startY + order.products.length * 10 + 30, `$${Tax}.00`);

  // total amount
  pdf.setFont("times", "bold");
  pdf.setFontSize(20);
  pdf.text(120, startY + order.products.length * 10 + 40, "Total:");
  pdf.text(
    160,
    startY + order.products.length * 10 + 40,
    `$${
      order.products?.reduce(
        (total, cartItem) =>
          total +
